---
{"dg-publish":true,"permalink":"/10-concurso-tj-ce/painel-tjce-2026/","title":"Painel de Controle - Concurso TJ-CE 2026","metatags":{"description":"Painel de Analista Judiciário – Área Apoio Especializado – Especialidade Ciência da Computação"},"tags":["concurso","tjce","controle","estudo"],"noteIcon":null,"updated":"2026-06-07T16:04:03.378-03:00","dg-note-properties":{"class":"classe-projeto","title":"Painel de Controle - Concurso TJ-CE 2026","tags":["concurso","tjce","controle","estudo"],"status":"ativo","prova":"2026-08-09","Priority":null,"Concluido":null,"cssclasses":null,"topics":null}}
---

# 🎯 Painel de Estudos: TJ-CE 2026 (Analista de Sistemas)

**Cargo:** Analista Judiciário – Área Apoio Especializado – Especialidade Ciência da Computação – Área Tecnologia da Informação - Sistemas (Código F06)
**Edital Base:** [[10-Concurso-TJ-CE/DJEA Matérias formatado\|DJEA - Edital TJ-CE]]
**Data da Prova:** 09/08/2026

---
## 🗓️ Cronograma Semanal (Ciclo Ação-Reflexão-Ação)

- [ ] **Segunda-feira:** Específicos (Governança e Processos) - PMBOK 7, ITIL v4, COBIT 2019, Licitações de TI.
- [ ] **Terça-feira:** Gerais (Legislação + Português) - Estatuto (Lei 9.826/74), Lei de Org. Judiciária, Questões FCC.
- [ ] **Quarta-feira:** Específicos (Arquitetura e PDPJ-Br) - Microsserviços, Spring, RabbitMQ, Keycloak.
- [ ] **Quinta-feira:** Específicos (DevOps, Cloud, Segurança) - CI/CD, Docker, K8s, LGPD, ISO 27001.
- [ ] **Sexta-feira:** Gerais (RLM + Direitos PCD) - Raciocínio Lógico, Lei 13.146/15 (Estatuto PCD).
- [ ] **Sábado:** Prática Discursiva + Revisão - Escrever 1 Redação (20-30 linhas) + Revisar mapas do Excalidraw.
- [ ] **Domingo:** Simulado + Análise de Erros - Resolver Prova FCC + Diagnóstico de pontos fracos.

---
## 📚 Edital Verticalizado (Checklist de Progresso)

### Conhecimentos Específicos (Peso 3)

#### Governança e Processos de TI
- [ ] PMBOK 7ª edição
- [ ] ITIL v4 e COBIT 2019
- [ ] Nova Lei de Licitações (Lei nº 14.133/2021) aplicada a TI
- [ ] Gestão de Riscos (ISO 31000)

#### Arquitetura e PDPJ-Br (Foco Total)
- [ ] Resoluções CNJ PDPJ-Br (Res. 335/2020, 396/2021, etc.)
- [ ] Arquitetura de Microsserviços e APIs RESTful
- [ ] Spring Boot, Spring Cloud, JPA/Hibernate
- [ ] Mensageria (RabbitMQ) e Autenticação (Keycloak/OAuth2)

#### DevOps, Cloud e Sistemas Operacionais
- [ ] CI/CD, Docker, Kubernetes, Rancher
- [ ] Cloud (IaaS, PaaS, SaaS) - AWS, Azure, GCP
- [ ] Linux (Red Hat/Oracle) e Windows Server

#### Segurança da Informação
- [ ] LGPD (Lei nº 13.709/2018)
- [ ] Normas ISO 27001 e 27002
- [ ] OWASP Top 10 e DevSecOps

#### Desenvolvimento e Banco de Dados (Revisão Prática)
- [ ] Padrões de Projeto (SOLID, DDD, MVC)
- [ ] PostgreSQL, Oracle, NoSQL
- [ ] React, Angular, Node.js (Foco em questões)

### Conhecimentos Gerais (Peso 1)
- [ ] **Legislação:** Lei Estadual nº 9.826/1974 (Estatuto dos Servidores CE)
- [ ] **Legislação:** Lei Estadual nº 16.397/2017 (Org. Judiciária CE)
- [ ] **Direitos PCD:** Lei 13.146/2015 e normas de acessibilidade
- [ ] **Português:** Interpretação, Sintaxe e Gramática (Banca FCC)
- [ ] **RLM:** Lógica de proposições, Estatística e Matemática básica

---
## 📊 Minhas Notas de Estudo (Dinâmico)

Crie novas notas na pasta `32-Formacao` com a tag `#tjce` para que elas apareçam automaticamente nesta lista de estudos.

| File                                                                                              | status | Última Atualização      |
| ------------------------------------------------------------------------------------------------- | ------ | ----------------------- |
| [[10-Concurso-TJ-CE/Painel-TJCE-Tecnico\|Painel-TJCE-Tecnico]]                                 | ativo  | 4:04 PM - June 07, 2026 |
| [[10-Concurso-TJ-CE/Painel-TJCE-2026\|Painel-TJCE-2026]]                                       | ativo  | 4:04 PM - June 07, 2026 |
| [[10-Concurso-TJ-CE/Cursos preparatórios para o TJCE\|Cursos preparatórios para o TJCE]]       | ativo  | 7:23 PM - May 29, 2026  |
| [[10-Concurso-TJ-CE/TJCE nível médio\|TJCE nível médio]]                                       | ativo  | 7:23 PM - May 29, 2026  |
| [[10-Concurso-TJ-CE/Estratégia para Analista Judiciário\|Estratégia para Analista Judiciário]] | ativo  | 7:22 PM - May 29, 2026  |
| [[10-Concurso-TJ-CE/DJEA Matérias formatado\|DJEA Matérias formatado]]                         | \-     | 7:22 PM - May 29, 2026  |

{ .block-language-dataview}