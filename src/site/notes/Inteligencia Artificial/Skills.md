...
| Modelo            | ✅                                                | ✅                                                                               |
| [[Inteligencia Artificial/parametros de LLM/System Prompt\|System Prompt]] | Todas instruções ficam no System Prompt          | System Prompt leve + skills ([[Inteligencia Artificial/Prompts\|Prompts]]) carregados [[Inteligencia Artificial/Workflows\|sob demanda]] |
...
- **[[Inteligencia Artificial/parametros de LLM/System Prompt\|System Prompt]] Mais Enxuto e Eficiente**: Enquanto os agentes tradicionais precisam ter todas as instruções no "[[Inteligencia Artificial/parametros de LLM/System Prompt\|System Prompt]]", as skills permitem que o "[[Inteligencia Artificial/parametros de LLM/System Prompt\|System Prompt]]" seja mais limpo, apenas indicando a disponibilidade dos "playbooks" externos (as skills). O conteúdo detalhado da skill é carregado apenas quando necessário ([[Inteligencia Artificial/Workflows\|Workflows]]).
...
- **Progressive Disclosure (Preservação da Janela de Contexto)**: As skills utilizam a janela de contexto de forma mais eficiente. Elas são carregadas apenas quando são necessárias, preservando a janela de contexto e tornando a IA mais efetiva. O "[[Inteligencia Artificial/parametros de LLM/System Prompt\|System Prompt]]" do agente apenas lista os nomes e descrições curtas das skills disponíveis, carregando o conteúdo completo de uma skill apenas se ela for solicitada ou ativada. Isso evita lotar a janela de contexto com informações que não são relevantes para a tarefa atual, o que acontece quando todas as instruções estão diretamente no "[[Inteligencia Artificial/parametros de LLM/System Prompt\|System Prompt]]" do agente.
...
- **Stitch Skills:** Quando integradas ao Google Stitch via MCP ([[Inteligencia Artificial/Model Context Protocol\|Model Context Protocol]]), permitem converter layouts em arquivos **design.md** (fonte da verdade), gerar componentes **React/TypeScript** modulares e criar o **Stitch Loop** (automação para gerar múltiplas páginas em segundo plano).
...
- [[Inteligencia Artificial/Repositórios de Skills\|Repositórios de Skills]]
