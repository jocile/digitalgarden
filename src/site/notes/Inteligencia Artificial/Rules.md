...
No ecossistema do Google [[Tecnico/Ferramentas/ferramentas de IA/Antigravity/Antigravity\|Antigravity]] e Cursor, as **rules (regras, papéis, preferências, [[Tecnico/UX design/requisitos\|requisitos]])** são definidas como **guardrails (trilhos de proteção) passivos e sempre ativos** que residem no prompt de sistema do agente. Diferente das [[Inteligencia Artificial/Skills\|Skills]], que são pacotes de conhecimento sob demanda, as regras funcionam como um código de conduta ou uma placa arquitetônica que o agente consulta antes e durante a escrita de código para garantir que nenhum padrão do projeto seja violado.
...
- **Workspace Rules:** São específicas para os [[Tecnico/UX design/requisitos\|requisitos]] de um projeto atual, geralmente armazenadas em arquivos como `.wgrad/rules.md` ou na pasta `.AGENTS/rules/` na raiz do projeto.

Ao contrário dos [[Inteligencia Artificial/Workflows\|Workflows]], que são macros acionadas manualmente pelo usuário via comando `/`, as regras são **acionadas automaticamente por gatilhos de contexto**.
